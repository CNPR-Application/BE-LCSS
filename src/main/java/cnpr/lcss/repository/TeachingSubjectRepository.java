...
public interface TeachingSubjectRepository extends JpaRepository<TeachingSubject, Integer> {
    ...
    @Query("select t from TeachingSubject t where t.teacher.account.username = ?1 and t.subject.subjectId = ?2")
    TeachingSubject findByTeacher_Account_UsernameAndSubject_SubjectId(String username, int subjectId);
}
